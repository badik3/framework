...
abstract class Model implements \JsonSerializable
{
    ...
    static public function getMeno($meno)
    {
        if ($meno == null) return null;

        self::connect();
        try {
            $sql = "SELECT * FROM " . self::getTableName() . " WHERE " . "meno" . "=?";
            $stmt = self::$connection->prepare($sql);
            $stmt->execute([$meno]);
            $model = $stmt->fetch();
            if ($model) {
    ...
    {
        if ($this->{self::$menoColum} == null) {
            return;
        }
        self::connect();
        try {
            $sql = "DELETE FROM " . self::getTableName() . " WHERE meno=?";
            $stmt = self::$connection->prepare($sql);
            $stmt->execute([$this->{self::$menoColum}]);
            if ($stmt->rowCount() == 0) {
                throw new \Exception('Model not found!');
            }
        } catch (PDOException $e) {
